...
2. Each render is reduced to one feature vector every 5.8 ms — 24 mel bands describing spectral shape, plus a "quiet" axis so silences match silences instead of correlating noise against noise. Alongside it, a separate set of decaying onset strengths: shape says *what* is sounding and holds steady through a syllable, so it is poor at saying *when* anything changed, and a short syllable is almost entirely change. The two are summed as costs rather than blended into one vector, which matters — two sustained frames score zero on the onset term and let the shape term decide alone. After [Ewert, Müller and Grosche](https://www.researchgate.net/publication/224461222_High_resolution_audio_synchronization_using_chroma_onset_features), who build the same thing on chroma.
